...
if (is.null(options$rpf_in) || is.null(options$mrna_in) || is.null(options$condition_vector_in) || is.null(options$riborexdeseq_result_path)){
  print_help(option_parser)
  stop("Please supply arguments (-r, -m, -c, -x), see --help \n", call.=FALSE)
}
...
# The input preparer labels the first contrast condition "treated" and the
# second "control". Name that comparison explicitly instead of relying on the
# last coefficient chosen by Riborex/DESeq2's current factor ordering.
results.deseq2 <- riborex(
  RNA,
  RIBO,
  contrastconditionsvector,
  contrastconditionsvector,
  contrast = c("cond", "treated", "control")
)
