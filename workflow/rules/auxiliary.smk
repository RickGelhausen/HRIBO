...
rule enrichAnnotation:
    ...
    threads: 1
    shell:
        "{SCRIPTS}/enrich_annotation.py -a {input.annotation} -o {output}"
...
rule samplesToExcel:
    ...
    threads: 1
    shell:
        "{SCRIPTS}/samples_to_xlsx.py -i {input} -o {output}"
...
rule createExcelTotalAnnotation:
    ...
    threads: 1
    shell:
        "{SCRIPTS}/generate_excel.py -t {input.total} -r {input.reads} -g {input.genome} -o {output}"
...
rule createExcelSummary:
    ...
    threads: 1
    shell:
        "{SCRIPTS}/generate_excel_reparation.py -t {input.total} -r {input.reads} -g {input.genome} -o {output}"
...
rule createExcelTotalAnnotationReadCount:
    ...
    threads: 1
    shell:
        "{SCRIPTS}/generate_read_table.py -r {input.reads} -t {input.total} -o {output}"
...
rule createOverviewTable:
    ...
    shell:
        """
        {SCRIPTS}/generate_excel_overview.py \
            -a {input.annotation} \
            -g {input.genome} \
            -t {input.totalreads} \
            {params.optional} \
