...
rule fastqctrimmed:
    ...
    output:
        html1="qc/2trimmed/{method}-{condition}-{replicate}-trimmed_fastqc_Q.html",
        html2="qc/2trimmed/{method}-{condition}-{replicate}-trimmed_fastqc_P.html",
        zip1="qc/2trimmed/{method}-{condition}-{replicate}-trimmed_fastqc_Q.zip",
        zip2="qc/2trimmed/{method}-{condition}-{replicate}-trimmed_fastqc_P.zip"
    conda:
        "../envs/fastqc.yaml"
    threads: 8
    params:
    ...
    shell:
        """
        mkdir -p qc/2trimmed;
        fastqc -o qc/2trimmed -t {threads} {input.reads1}; mv qc/2trimmed/{params.prefix1}_fastqc.html {output.html1}; mv qc/2trimmed/{params.prefix1}_fastqc.zip {output.zip1}
        fastqc -o qc/2trimmed -t {threads} {input.reads2}; mv qc/2trimmed/{params.prefix2}_fastqc.html {output.html2}; mv qc/2trimmed/{params.prefix2}_fastqc.zip {output.zip2}
        """
...
rule multiqc:
    input:
        ...
        expand("qc/2trimmed/{method}-{condition}-{replicate}-trimmed_fastqc_Q.html", zip, method=samples["method"], condition=samples["condition"], replicate=samples["replicate"]),
        expand("qc/2trimmed/{method}-{condition}-{replicate}-trimmed_fastqc_P.html", zip, method=samples["method"], condition=samples["condition"], replicate=samples["replicate"]),
