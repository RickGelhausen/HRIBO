...
psite_offsets = None
psite_offsets_path = None
if mode == "psite_out_of_range":
    temporary = work_dir / "tmp"
    temporary.mkdir()
    rois = temporary / "plastid_rois.txt"
    rois.write_text("fixture ROI\n")
    prefix = temporary / "plastid"
    ...
    psite_offsets_path = Path(str(prefix) + "_p_offsets.txt")
    psite_offsets = psite_offsets_path.read_text()
    expected_offsets = {"22": "13", "23": "13", "28": "12", "default": "13"}
    observed_offsets = {
        fields[0]: fields[1]
        for line in psite_offsets.splitlines()
        if line and not line.startswith("#") and (fields := line.split("\t"))[0] != "length"
    }
    if observed_offsets != expected_offsets:
        print("occupancy received unsafe P-site offsets", file=sys.stderr)
        sys.exit(91)
    (work_dir / "offsets_seen_by_occupancy.txt").write_text(psite_offsets)
...
if mode == "bad_offsets":
    ...
    offsets = "length\tp_offset\n28\t28\n"
elif psite_offsets is not None:
    offsets = psite_offsets
offset_output = work_dir / "p_site_offsets.txt"
if psite_offsets_path is None:
    offset_output.write_text(offsets)
else:
    shutil.copy(str(psite_offsets_path), str(offset_output))
...
assert Path(arguments[0]).name == "plastid_rois.txt"
assert Path(arguments[-1]).name == "reads.bam"
output = Path(arguments[1] + "_p_offsets.txt")
output.write_text(
    "## generated by fake Plastid\n\n"
    "length\tp_offset\n"
    "22\t22\n"
    "23\t27\n"
    "28\t12\n"
...
"""
    ...
    psite.chmod(psite.stat().st_mode | stat.S_IXUSR)
...
def execute(fixture, mode):
    command, paths, _ = fixture
    output_dir = fixture[2]
    environment = {
        **os.environ,
        "FAKE_REPARATION_MODE": mode,
        "PATH": os.pathsep.join(
            [str(paths["psite"].parent), os.environ.get("PATH", "")]
        ),
    }
...
def production_rule_run(reparation_run, snakemake_command):
    ...
    (fake_bin / "psite").symlink_to(paths["psite"])
...
def test_reparation_replaces_impossible_psite_estimates_before_prediction(
    reparation_run,
):
    result = execute(reparation_run, "psite_out_of_range")
    _, _, output_dir = reparation_run

    assert result.returncode == 0, result.stderr
    offsets = (output_dir / "p_site_offsets.txt").read_text()
    assert "22\t13\n" in offsets
    assert "23\t13\n" in offsets
    assert "28\t12\n" in offsets
    assert "22\t22\n" not in offsets
    assert "23\t27\n" not in offsets
    assert "before REPARATION occupancy" in offsets
    assert stat.S_IMODE((output_dir / "p_site_offsets.txt").stat().st_mode) == 0o640
    assert (output_dir / "offsets_seen_by_occupancy.txt").read_text() == offsets
    assert "offset of 22 nt for 22-nt reads" in result.stderr
    assert "offset of 27 nt for 23-nt reads" in result.stderr
    assert "default offset (13 nt)" in result.stderr
    assert "before REPARATION calculated occupancy and predictions" in result.stderr
