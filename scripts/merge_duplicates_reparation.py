    ...
    for key in geneDict.keys():
        ...
        for row in geneDict[key]:
            ...
            if "Prob" in attributes:
                proba.append(float(attributes[attributes.index("Prob")+1]))

        proba = sum(proba) / len(proba)
        attribute = "ID="+key+";Name="+key+";ORF_type="+",".join(orftype)+";Evidence="+" ".join(evidence)+";Prob=" + str(proba)
